...
//    float lz = randFloat(l, l + (l/1.5)); //
    ...
    //PINNE
    gl::color(1.f, 1.f, 1.f, 0.2);
    gl::rotate(Vec3f(10.f, -(numPinne-1) * distPinne / 2, 0.f));
//      rot.y = (-(numPinne-1) * distPinne / 2);
    ...
    for (int k=0; k<numPinne; k++) {
        ...
        for (int j=0; j<numTentacoli; j++) {
            gl::pushMatrices();
            float a = (mPerlin.noise(count*noiseVel.x + j+k+1)-0.4)*10.782;
            float b = (mPerlin.noise(count*noiseVel.y + j+k+1)-0.5)*10.582;
            float c = (mPerlin.noise(count*noiseVel.z + j+k+1)-0.6)*10.682;
...
//                rot.x = (c);
                ...
                gl::translate(l*0.9f, 0.f, 0.f);
                gl::scale(0.85, 0.85f, 0.85f);
                glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
